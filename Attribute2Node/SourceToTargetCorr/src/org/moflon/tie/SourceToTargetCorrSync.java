...
public class SourceToTargetCorrSync extends DebugSynchronizationHelper {
	...
	private void saveResult(String direction) {
		getSrc().eResource().setURI(
				URI.createFileURI("sync." + direction + ".src.xmi"));
		getTrg().eResource().setURI(
				URI.createFileURI("sync." + direction + ".trg.xmi"));
		getCorr().eResource().setURI(
				URI.createFileURI("sync." + direction + ".corr.xmi"));
		...
		saveSynchronizationProtocol("instances/sync." + direction
				+ ".protocol.xmi");
	}
}